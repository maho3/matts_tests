...
setup_logger(glbcfg['logdir'], name='jax1lpt')
...
def run_density(ic, L, N, ai, af, cpar, transfer="EH"):
    ...
    logging.info("Generating initial density...")
    rho_init = utils.generate_initial_density(L, N, cpar, ai, ic, transfer)

    # JAX-2LPT model
    logging.info("Initializing JAX-1LPT solver...")
    fwd = lpt.Jax1LptSolver(box, cpar, ai, af, with_velocities=True)
...
def main():
    ...
    outdir = pjoin(glbcfg["wdir"], "jax1lpt",
    ...
    if args.matchIC:
        ...
        path_to_ic = pjoin(glbcfg['wdir'],
                           f'quijote/wn/wn-N{N}/wn_{args.lhid}.dat')
